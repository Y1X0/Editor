...
#
# **١٠٠٠ هون هي القيمة الاسمية لـHLG — احتياطي موثَّق للنداء المباشر،
# مش القيمة المنتَجة.** المنتَجة `geometry.tonemap_npl` بـ`config.json`
# وهي **٧٠٠، معايَرة بالعين** على مشهد خارجي نهاري ساطع. شوف
# `ISSUES.md` / CAL-1: عند ١٠٠٠ وفوق الصورة بتبهت، وتحت ٤٠٠ التباين
# بينشدّ والمناطق الغامقة بتفقد تفاصيلها.
#
# والفرق بين «اسمية» و«معايَرة» مش شكليًا: قِسنا السلسلة عند `npl=100`
# فطلعت نتيجتان مريحتان (لا تأثير على SDR، وأبيض الكابشن بينجو)
# والاتنين **غلط**. شوف `CLAUDE.md`، «أداة القياس بتنكسر أكتر من
# الكود».
DEFAULT_NPL = 1000
