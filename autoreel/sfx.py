...
# **`config.json` هو مصدر خريطة الأحداث بالمنتج.** هالجدول احتياطي
# للنداء المباشر بلا `cfg` (وحدات وتجارب)، و`merged_config` بتخلّي
# الconfig يفوز على كل مفتاح. `tests/test_config_wiring.py` بيثبّت إن
# جدول الconfig بيغطّي كل نوع بـ`PRIORITY` وإن كل قيمة فيه بتوصل
# للمخرَج فعلًا.
DEFAULTS = {
    "enabled": True,
