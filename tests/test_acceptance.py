"""
اختبارات القبول E1..E8 على مسار الإنتاج الحقيقي.

الحالة بعد المرحلة ٦ (الصوت بنفس التشغيلة):

    E1 ✅  عدد الإطارات = خطة الإطارات      — CR-4 انقرض
    E7 ✅  هوية كل إطار، بلا تكرار ولا نقص  — CR-5 انقرض
    E2 ✅  ١.٩٨ms = أرضية القياس، تراكم صفر — CR-2 انقرض
    E8 ✅  الزوم لكل مقطع، قبل وبعد
    E3 ❌  متوقَّع: مسار الكابشن ما انتغيّر بعد (المرحلة ٧)

**اختبار ما شفناه بيفشل مش اختبار.** كل واحد من E1/E2/E3/E7 انشغّل
وانشاف فاشلًا على المعمارية القديمة **قبل** ما يتغيّر أي كود إنتاج،
وE8 انشاف ناجحًا.
...
def _expected_click_times(run):
    """
    وين المفروض تقع كل نقرة بالمخرَج.

    **بداية المقطع هي `round(a·fps)/fps` مش `a`.** أول نسخة من هالفحص
    استعملت `a` العائمة، فطلع انزياح ثابت ١٣.٣ms = بالضبط
    `1.780 − 53/30`. الفرق كان بالتوقّع مش بالمسار: المعمارية بتقصّ على
    شبكة الإطارات، والتوقّع لازم يجي من نفس الشبكة — نفس درس E7.

    (على المعمارية القديمة كان `a` تقريبًا صح لأن `-ss` بتقصّ بالزمن
    العائم. الفحص كان بيشفّر عقد المعمارية القديمة.)
    """
    fps = run["fps"]
    return [run["offsets"][i] / fps + (CLICK_AT[i] - round(a * fps) / fps)
            for i, (a, _) in enumerate(run["segs"])]


def test_e2_audio_has_no_cumulative_drift(run):
    """
    نقرة معروفة بكل مقطع، ومكانها المتوقَّع مشتقّ من خطة الإطارات.

    الانزياح كان بيتراكم لأن كل مقطع بينرمّز AAC لحاله وbpriming padding
    بينضاف مع كل `concat` — مقاس ≈١٩ms لكل قطعة، ١٥٢ms عند ٨ مقاطع.
    """
    got = click_times(run["out"])
    want = _expected_click_times(run)
...
def test_e2_each_click_lands_in_the_frame_that_shows_it(run, frames_dir):
    """
    **فحص مستقل تمامًا عن صيغة التوقّع فوق.**

    ما بيحسب أي زمن متوقَّع: بياخد إطار المصدر اللي فيه النقرة، بيلاقي
    وين ظهر هالإطار بالمخرَج (من هوية الإطار)، وبيتأكد إن النقرة سمعت
    عنده. يعني بيفحص **تزامن الصوت مع الصورة** مباشرة.

    لو غلطت بصيغة التوقّع بـ`_expected_click_times`، هالفحص بيمسكها —
    وهاد سبب وجوده: الفحص اللي بيتشارك افتراضًا مع اللي بيفحصه ما
    بيقدر يكشف غلط بهداك الافتراض.
    """
    ids = read_identities(frames_dir)
    got = click_times(run["out"])
    assert len(got) == len(CLICK_AT), f"{len(got)} نقرة من {len(CLICK_AT)}"
    tol = 1.5 / run["fps"]
    bad = []
    for i, t in enumerate(CLICK_AT):
        src_frame = int(t * run["fps"])
        if src_frame not in ids:
            bad.append((i, src_frame, "إطار المصدر مش بالمخرَج"))
            continue
        want = ids.index(src_frame) / run["fps"]
        if abs(got[i] - want) > tol:
            bad.append((i, round(got[i], 4), round(want, 4)))
    assert not bad, f"نقرات مش بإطارها: {bad}"
...
# كان هون تلات حرّاس بيثبّتوا اللي مكسور. كلهن **انسحبوا عن قصد** لما
# انقرض خللهن، حسب عقد المرحلة ٣: لو مرق حارس، يا الخلل انصلّح فنشيل
# سطره، يا الفحص فقد أسنانه.
#
#   CR-4 (+١ إطار)         -> راح بالمرحلة ٥ (مسار الفيديو الواحد)
#   CR-5 (تكرار أول إطار)   -> راح بالمرحلة ٥ (القصّ بفهرس الإطار)
#   CR-2 (انزياح متراكم)    -> راح بالمرحلة ٦ (ترميز AAC واحد)
#
# E1 وE7 وE2 صاروا حرّاسهن من هلأ ورايح. الوحيد اللي لسا مكسور هو
# توقيت الكابشن (E3)، وهو بيحرس حاله: بيفشل اليوم وبينجح بالمرحلة ٧.
