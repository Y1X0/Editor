...
الحالة بعد المرحلة ٥ (مسار الفيديو الواحد):

    E1 ✅  عدد الإطارات = خطة الإطارات      — CR-4 انقرض
    E7 ✅  هوية كل إطار، بلا تكرار ولا نقص  — CR-5 انقرض
    E8 ✅  الزوم لكل مقطع، قبل وبعد
    E2 ❌  متوقَّع: مسار الصوت ما انتغيّر بعد   (المرحلة ٦)
    E3 ❌  متوقَّع: مسار الكابشن ما انتغيّر بعد (المرحلة ٧)

**اختبار ما شفناه بيفشل مش اختبار.** كل واحد من E1/E2/E3/E7 انشغّل
وانشاف فاشلًا على المعمارية القديمة قبل ما يتغيّر أي كود إنتاج، وE8
انشاف ناجحًا. `test_the_audio_drift_is_still_there` بيحرس اللي لسا
مكسور؛ حرّاس CR-4 وCR-5 **انسحبوا عن قصد** لما انقرض خللهن (شرح
بالدالة نفسها).
...
def test_the_audio_drift_is_still_there(run):
    """
    **بيوثّق اللي لسا مكسور، وبيحرس أسنان الفحص المقابل.**

    كان هون تلات تأكيدات. اتنين **انسحبوا عن قصد** بالمرحلة ٥ لأن
    خللهن انقرض:

      CR-4 (+١ إطار)      -> راح مع مسار الفيديو الواحد
      CR-5 (تكرار أول إطار) -> راح مع القصّ بفهرس الإطار

    والعقد كان صريحًا: لو مرق واحد منهن، يا الخلل انصلّح فنشيل سطره عن
    قصد، يا الفحص فقد أسنانه. هون الأول — E1 وE7 صاروا يمرقوا، وهنّ
    حرّاسهن من هلأ ورايح.

    الباقي: انزياح الصوت المتراكم. لسا موجود لأن مسار الصوت **ما
    انتغيّر ولا حرف** بالمرحلة ٥ (سقالة `_legacy_audio`). الدليل إن
    الأرقام مطابقة تمامًا لما كانت قبل التغيير:
    `[21.3, 40.0, 58.7, 77.3, 96.0, 114.7, 133.3, 152.0]` ms.
    بينشال هالفحص بالمرحلة ٦.
    """
    ...
    assert abs(errs[-1] - errs[0]) > DRIFT_ACCUM_MS, (
        f"E2 مرق على مسار الصوت القديم (تراكم {errs[-1]-errs[0]:+.1f}ms) — "
        f"يا الانزياح انصلّح يا الفحص فقد أسنانه")
