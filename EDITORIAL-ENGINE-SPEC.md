...
> ممنوع قبل الخطوة ٣.
>
> **الحالة: ARCHITECTURE APPROVED** — بقيد واحد معلَّق:
> `models/timeline.py::shot_in_frame` (§١.٤) **قرار معماري مستقل**،
> بيتاخد لحاله قبل المرحلة ١، لا بيمرق ضمن مرحلة تانية.
>
> **ولا التفاف مقبول:** حطّ النافذة داخل `concept` أو `strategy` أو
> أي حقل نصّي **مرفوض** — بيكسر الفصل اللي الوثيقة كلها قائمة عليه،
> وبيرجّع الزمن لجيب الوكيل من الباب الخلفي.
...
3. **المراجعة لازم تُثبت نفسها — وهاد الحارس الأهم:**
   ...
   **وولا محاولة لإصلاح الإصلاح.** الفشل بينهي الدورة، ما بيفتح
   وحدة جديدة:

   ```
   Draft ──► Revision ──► Proof ─┬─ PASS ──► FinalPlan
                                 └─ FAIL ──► Draft  (وانتهى)
   ```

   لا `Draft → Revision → Revision → …`. الحلقة اللي بتحاول تصلّح
   فشلها هي نفسها الحلقة اللي ما بتنتهي، وعليها فحص: محاولة تانية
   **بترمي**.
...
| **ولا تمثيل زمني بديل** | نفس القائمة بعد سدّ ثغرتها (§٦.١) | ضيف `in_frame` |
...
### ٦.١ ⚠️ ثغرة مقيسة بالقائمة نفسها — شرط للمرحلة ٣

القاعدة الصح كما ثبّتها المالك: **ولا منظومة تانية — نختبر القائمة
نفسها.** فقِسناها، وطلعت ناقصة.

الحارس بيقارن **أسماء بالضبط**:

```python
bad = set(dto.model_fields) & FORBIDDEN_FIELD_NAMES
```

فأي اسم مش بالقائمة حرفيًّا **بيمرق**. والقياس على الأسماء التسعة:

| الاسم | بالقائمة؟ |
|---|---|
| `start` · `end` · `duration` · `frame` · `frames` · `timestamp` | ✅ موجودة |
| **`in_frame`** · **`out_frame`** · **`timecode`** | ❌ **ناقصة** |

يعني حقل اسمه `in_frame` بـ`ShotProposal` **بيعدّي اليوم بلا ولا
فشل**. وخطورته مش نظرية: الحقل اللي عم نقترحه بالـ`Timeline` اسمه
`shot_in_frame` حرفيًّا — فهو أقرب اسم رح يمدّ إيده إله أي حدا بيجي
بعدنا وبدّه «يمرّر النافذة بالخطة».

**التصليح تلات كلمات على `FORBIDDEN_FIELD_NAMES`، وما بينعمل الآن**
لأنه بيلمس كود المنتَج قبل الـbaseline. **شرط للمرحلة ٣**، ومعه فحص
بيمشي على الأسماء التسعة كلها لا على `duration` وحدها.
...
| ١ | `shot_in_frame` — **قرار مستقل، بإذن صريح ومنفصل** | ⚠️ عقد مجمّد |
| ٢ | `StoryModel` + `editorial_function` | نعم |
| ٣ | حقول الاستراتيجية + `repeat_reason` + سدّ §٦.١ | نعم |
